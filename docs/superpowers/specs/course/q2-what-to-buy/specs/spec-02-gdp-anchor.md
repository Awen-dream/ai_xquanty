...
1. 先做数据合法性检查：
   - `assert len(gdp_latest) == 10`
   - `assert gdp_latest.loc["USA"] > gdp_latest.loc["DEU"]`
   - `assert gdp_latest.loc["CHN"] > gdp_latest.loc["JPN"]`
   - `assert abs(csi300_norm.iloc[0] - 100.0) < 1e-9`
   - `assert abs(qqq_norm.iloc[0] - 100.0) < 1e-9`
2. GDP 横向柱状图。
3. 打印：「最直觉的思路：经济最强的国家，资本市场应该最有潜力。」
4. 打印：「数据说了算：锁定中国（沪深300 ETF）和美国（纳斯达克100 ETF）」。
5. 两只 ETF 归一化走势对比图。
6. 打印：「但一个新问题出现了——如果中美股市同时下跌怎么办？」
