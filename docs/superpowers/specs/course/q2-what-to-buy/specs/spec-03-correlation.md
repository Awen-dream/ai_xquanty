...
1. 先做数据合法性检查：
   ...
   - `assert set(snapshot.symbols) == {"510300.SS", "513100.SS", "518880.SS"}`
2. 相关性热力图。
3. 打印相关性解读：
   - 「沪深300 与 纳指100 的相关性：X.XX —— 两个股票市场有一定联动，但并非完全同步」
   - 「沪深300 与 黄金 的相关性：X.XX —— 黄金与股市低相关，股市跌的时候它不一定跌」
   - 「纳指100 与 黄金 的相关性：X.XX」
4. 打印组合 vs 单押的对比表，使用 `metrics.to_string(index=False)`。
5. 固定窗口下的参考结果应四舍五入为：
...
6. 累计收益率对比图。
7. 打印关键认知：「数据说了算：分散风险的关键不是多买几个，而是买涨跌不同步的。」
8. 打印标的池构建结果：`「标的池构建完成: {snapshot.symbols}」`
