...
4. 止损阈值敏感性：
   ...
   - “无止损”请作为表格最后一行统一展示，不要游离成表外单独一句 print

5. 计算每组的夏普比极差（最大夏普比 - 最小夏普比），打印对比：
   - RiskParity 夏普比极差
   - TopNRanking 夏普比极差
   - 标注哪个是"高原型"（极差小），哪个是"山峰型"（极差大）

6. 画参数敏感性折线图（figsize 15x5，1x3 子图）：
...
执行成功的标志：
- `assert len(rp_sensitivity) == 6`
- `assert len(tnr_sensitivity) == 6`
- `assert len(sl_sensitivity) == 7`（“无止损”是额外展示行，不计入 `sl_sensitivity` 字典）
- 三组表各有正确的行列数
- 折线图 3 个子图，y 轴范围一致
- 夏普比极差有明确数值，可判断高原型 vs 山峰型
- `assert rp_range < tnr_range`
