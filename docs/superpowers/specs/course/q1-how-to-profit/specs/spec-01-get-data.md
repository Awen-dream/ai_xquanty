...
2. 在 notebook 中创建代码单元格，获取沪深300ETF固定“最近 5 年快照窗口”的历史日线数据并可视化
...
3. 使用 yfinance 获取数据：
   ...
   - 起始日期：2021-07-28
   - 数据截止日期：2026-07-27（对应 2026-07-28 视角下最近 5 年的固定快照窗口）
   - 不要使用 `today`、`now()` 或默认截止到最新数据的下载方式
   - `yfinance` 的 `end` 参数是排他边界；代码中可用 `2026-07-28` 作为下载结束参数，再截取到 `2026-07-27`
   - 传入参数 `auto_adjust=True, multi_level_index=False` 避免警告和多级列名
4. 将数据存储在变量 `df` 中
5. 确保索引为 DatetimeIndex，列名包含 Close
6. 下载后如果 `df` 为空，必须立刻抛出明确错误，提示学员检查网络、代理或数据源状态，不要继续执行后续单元
