...
4. 注意处理：
   ...
   - 跳过买入的月份，`signal=False`，累计成本和累计份额都保持不变
   - 计算收益率时，先把整列收益率初始化为 0，只对累计成本 `> 0` 的月份做除法计算
5. 分别用 MA10、MA20、MA60 三个参数运行回测

## 结果呈现

1. 先做最小验证：
   - `assert 'signal' in result_ma.columns`
   - `assert result_ma['signal'].dtype == bool`
2. 打印每个参数的结果，格式：`MA10: 收益率 X.XX%, 买入 N/M 次`
3. 打印简单定投收益率作为对照
4. 画一张对比图（figsize 12×6）：
   - 简单定投收益率曲线（灰色虚线）
